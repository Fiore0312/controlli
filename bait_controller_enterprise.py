#!/usr/bin/env python3
"""
BAIT Service - Enterprise Controller
====================================

Controller enterprise ottimizzato per processing CSV e generazione dati dashboard.
Sistema robusto con error handling, logging avanzato e performance optimization.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'logs/bait_enterprise_{datetime.now().strftime("%Y%m%d_%H%M")}.log'),
...
class BAITEnterpriseController:
    """Enterprise Controller per processing completo sistema BAIT"""
    ...
    def __init__(self, data_directory: str = ".", config: Dict = None):
        ...
        logger.info("🚀 BAIT Enterprise Controller initialized")
    ...
    def detect_file_encoding(self, file_path: Path) -> str:
        ...
        try:
            ...
            logger.info(f"📄 {file_path.name}: encoding={encoding}, confidence={confidence:.2f}")
            ...
            logger.warning(f"⚠️ Encoding detection failed for {file_path}: {e}")
            return 'utf-8'
    ...
    def load_csv_robust(self, file_path: Path, expected_columns: List[str] = None) -> pd.DataFrame:
        ...
        logger.error(f"❌ Failed to load {file_path} with any encoding/separator combination")
        return pd.DataFrame()
    ...
    def _clean_dataframe(self, df: pd.DataFrame, source_file: str) -> pd.DataFrame:
        ...
        logger.info(f"🧹 Cleaned {source_file}: {len(df)} rows after cleaning")
    ...
    def _parse_datetime_column(self, series: pd.Series, column_name: str) -> pd.Series:
        ...
        for fmt in datetime_formats:
            try:
                ...
                if not parsed.isna().all():
                    logger.debug(f"📅 Parsed {column_name} with format {fmt}")
                    return parsed
            except:
                continue
        ...
        try:
            ...
            if not parsed.isna().all():
                logger.debug(f"📅 Auto-parsed {column_name}")
                return parsed
        except:
            pass
        
        logger.warning(f"⚠️ Could not parse datetime column: {column_name}")
        return series
    ...
    def process_all_files(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        logger.info(f"🔄 Starting enterprise processing [ID: {processing_id}]")
        ...
        try:
            ...
            if not force_refresh:
                ...
                if cached_result:
                    logger.info("⚡ Using cached result for performance")
    ...
    def _process_business_rules(self, data_frames: Dict[str, pd.DataFrame]) -> List[Dict]:
        ...
        logger.info(f"🧠 Business rules processed: {len(alerts)} alerts generated")
    ...
    def _generate_demo_result(self) -> Dict:
        ...
        logger.info("🎭 Generating demo data for demonstration")
...
def main():
    ...
    print("🚀 BAIT Enterprise Controller v3.0")
    print("=" * 50)
    ...
    try:
        ...
        print(f"  ⏱️ Processing time: {result['metadata'].get('processing_duration', 0):.2f}s")
        print(f"  💰 Estimated losses: €{result['kpis_v2']['system_kpis']['estimated_losses']}")
